...
public class RecruitService {
    ...
    private Specification<Recruit> createSpecification(Map searchMap) {
        ...
        return new Specification<Recruit>() {
            ...
            @Override
            public Predicate toPredicate(Root<Recruit> root, CriteriaQuery<?> query,
                                         CriteriaBuilder cb) {
                List<Predicate> predicateList = new ArrayList<Predicate>();
                // ID
                if (searchMap.get("id") != null && !"".equals(searchMap.get("id"))) {
                    predicateList.add(cb.like(root.get("id").as(String.class),
                ...
                if (searchMap.get("salary") != null && !"".equals(searchMap.get("salary"))) {
                    predicateList.add(cb.like(root.get("salary").as(String.class),
                ...
                if (searchMap.get("type") != null && !"".equals(searchMap.get("type"))) {
                    predicateList.add(cb.like(root.get("type").as(String.class),
                        "%" + (String) searchMap.get("type") + "%"));
                }
                // 办公地址
                ...
                if (searchMap.get("eid") != null && !"".equals(searchMap.get("eid"))) {
                    predicateList.add(cb.like(root.get("eid").as(String.class),
                        "%" + (String) searchMap.get("eid") + "%"));
                }
                // 状态
                ...
                if (searchMap.get("label") != null && !"".equals(searchMap.get("label"))) {
                    predicateList.add(cb.like(root.get("label").as(String.class),
    ...
    public List<Recruit> findTop4ByStateOrderByCreationTime(String state) {
        return recruitDao.findTopByStateOrderByCreatetimeDesc(state);
    }

    public List<Recruit> findTop12ByStateOrderByCreationTimeDesc() {
